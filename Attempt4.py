...
for epoch in range(1):  # loop over the dataset multiple times
    ...
    for j in range((int)(trainDataX.shape[0]/batch_size)): # Loop over batches 
        ...
        for i in range(batch_size): # Loop over each data point (use SGD for gradient)
            ...
            loss = lossFunction(torch.Tensor(trueWeightsR), torch.Tensor(trueWeightsI), weightsR, weightsI, trainDataX[j*batch_size+i, :])
        ...
        with torch.no_grad():
            if printTestLoss_Index%50==1:
                ...
                a = a + 1
            printTestLoss_Index=printTestLoss_Index+1
...
#Store model
torch.save(net, r"\Users\user1\Desktop\DiplomaThesis-Tzikas\TinyYoloDoA4.pth") 
...
# What is new?
# A mini-batch of 50 has been set. The sum of losses per 50 examples is calculated and a single optimize step is taken using the mean loss of the mini-batch,
