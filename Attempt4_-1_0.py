...
def toInputImage(directions, nrows, ncols):
    ...
    image = np.zeros([nrows, ncols])+0.5

    t=0
    for i in range(len(directions)):
        index = (int)(np.floor((directions[i] - minAngle) / by))
        if index < nrows*ncols:
            image[np.unravel_index(index, (nrows, ncols), order="C")] = 1.0 if i == 0 else 0.0
            indices[t, :] = np.unravel_index(index, (nrows, ncols), order="C")
        else:
            image[np.unravel_index(index-1, (nrows, ncols), order="C")] = 1.0 if i == 0 else 0.0
            indices[t, :] = np.unravel_index(index-1, (nrows, ncols), order="C")
        t = t+1

    for m in range(nrows):
        for n in range(ncols):
            if (image[m,n]!=1.0 and image[m,n]!=0.0):
...
#Store model
torch.save(net, r"\Users\user1\Desktop\DiplomaThesis-Tzikas\TinyYoloDoA4_-1_0.pth") 
