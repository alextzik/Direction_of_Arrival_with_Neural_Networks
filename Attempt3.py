...
optimizer = optim.SGD(net.parameters(), lr=0.005, momentum=0.0) # If training error rises and falls reduce learning rate

trainLoss = np.zeros((int)(trainDataX.shape[0]/100)) # Train loss for every batch 
testLoss = np.zeros((int)(trainDataX.shape[0]/(50*100))) # Loss in test set for weights updated after every batch

trainLossPerExample = np.zeros(trainDataX.shape[0])
pointer=0
...
for epoch in range(1):  # loop over the dataset multiple times
    ...
    running_loss = 0.0
    for j in range((int)(trainDataX.shape[0]/100)): # Loop over batches 
        running_loss = 0.0
        for i in range(100): # Loop over each data point (use SGD for gradient)
            inputs=torch.Tensor(toInputImage(trainDataX[j*100+i, :], nrows=448, ncols=448))
            #inputs=inputs.float()
            ...
            for t in range(numOfElements):
                trueWeightsR[t] = trainDataY[j*100+i, t+index]
                trueWeightsI[t] = trainDataY[j*100+i, t+1+index]
                index +=1

            loss = lossFunction(torch.Tensor(trueWeightsR), torch.Tensor(trueWeightsI), weightsR, weightsI, trainDataX[j*100+i, :])
            ...
            trainLossPerExample[pointer] = loss.item()
            pointer = pointer + 1
            print(j, i, running_loss)
        trainLoss[j]=running_loss/100

        # Find error in test set after training in specific batch
        with torch.no_grad():
            if printTestError_Index%50==1:
                test_loss=0
...
#Store model
torch.save(net, r"\Users\user1\Desktop\DiplomaThesis-Tzikas\TinyYoloDoA3.pth") 
...
plt.figure()
plt.plot(trainLossPerExample, color='b')
